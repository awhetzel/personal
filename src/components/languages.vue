                ...
                <v-col v-for='logo in chunk' :key='logo.path' sm="12" md="40">
                    <v-tooltip left>
                        <template v-slot:activator="{ on }">
                            <v-img :src=image_path(logo) max-width="100px" contain v-on="on" class="ma-2"> </v-img>
                        </template>
                        <span>{{ logo.toolTip }}</span>
                    </v-tooltip>
                </v-col>
            </div>
        </v-row>
...
export default {
    ...
    computed: {
        libraryLogoChunks() {
            return this.$store.getters.libraryLogos
        },
    ...
    methods: {
        image_path(logo) {
            return require(`../assets/${logo.path.toLowerCase()}`)
        }
    }
}
